...
# ==============================
def carregar_tabulacoes_n3_jira():
    ...
    with engine.connect() as cnxn:
        ...
        FROM
            TbaseTabuladorZendesk b WITH(NOLOCK) 
            LEFT JOIN TtbuladorZendeskIntercambio t WITH(NOLOCK) ON b.Id = t.FkTbaseTabuladorZendesk
            LEFT JOIN TemissoresZendeskDescricao d WITH(NOLOCK) ON b.FkTipoEmissor = d.Id
...
# ==============================
def executar_e_salvar_tabulacoes_n3_jira():
    print("Iniciando processo de extracao e salvamento Tabulacoes N3 Jira...")
    
    try:
        # 1. Obter dados novos do SQL
        df_novo = carregar_tabulacoes_n3_jira()
        
        if df_novo.empty:
            print("Nenhum dado novo retornado para o período.")
            return

        caminho_parquet = os.path.join(PASTA_DESTINO, "import_tabulacoes_n3_jira.parquet")
...
if __name__ == "__main__":
    executar_e_salvar_tabulacoes_n3_jira()