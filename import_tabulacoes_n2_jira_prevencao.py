...
# ==============================
def carregar_tabulacoes_n2_jira_prevencao():
    
    engine = login.connProDockTech
    with engine.connect() as cnxn:
        query_sql = f"""
        SELECT
            'N2: JiraPrevencao' AS Tabulador,
            t.Id,
            t.DtCadastro,
            t.DataSolicitacao,
        ...
        FROM
            TtbuladorJiraPrevencao t WITH(NOLOCK)
...
def executar_e_salvar_tabulacoes_n2_jira_prevencao():
    print("Iniciando processo de extracao e salvamento Tabulacoes N2 JiraPrevencao...")
    
    try:
        # 1. Obter dados novos do SQL
        df_novo = carregar_tabulacoes_n2_jira_prevencao()
        
        if df_novo.empty:
            print("Nenhum dado novo retornado para o período.")
            return

        caminho_parquet = os.path.join(PASTA_DESTINO, "import_tabulacoes_n2_jira_prevencao.parquet")
...
if __name__ == "__main__":
    executar_e_salvar_tabulacoes_n2_jira_prevencao()
