...
# ==============================
def carregar_tabulacoes_n3_visa():
    ...
    with engine.connect() as cnxn:
        ...
        FROM
            TbaseTabuladorEmissoresVisa b WITH(NOLOCK) 
            LEFT JOIN TtbuladorEmissorVisa t WITH(NOLOCK) ON b.Id = t.FkTbaseTabuladorEmissoresVisa
            LEFT JOIN TemissoresDescricaoVisa d WITH(NOLOCK) ON b.FkTipoEmissor = d.Id
...
# ==============================
def executar_e_salvar_tabulacoes_n3_visa():
    print("Iniciando processo de extracao e salvamento Tabulacoes N3 Visa...")
    
    try:
        # 1. Obter dados novos do SQL
        df_novo = carregar_tabulacoes_n3_visa()
        
        if df_novo.empty:
            print("Nenhum dado novo retornado para o período.")
            return

        caminho_parquet = os.path.join(PASTA_DESTINO, "import_tabulacoes_n3_visa.parquet")
...
if __name__ == "__main__":
    executar_e_salvar_tabulacoes_n3_visa()