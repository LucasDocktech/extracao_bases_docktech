...
# ==============================
def carregar_tabulacoes_OnboardingPJ_Manual():
    engine = login.connProDockTech
    with engine.connect() as cnxn:
        query_sql = f"""
        SELECT
            'OnboardingPJ_Manual'AS Tabulador,
            t.DtCadastro AS Tabulacao_DtCadastro,
            t.DataDaEntrada AS Tabulacao_DtDaEntrada,
            t.DataInicio AS Tabulacao_DataInicio,
            t.Cnpj AS CNPJ,
            ...
            t.Fila,
            UPPER(LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(REPLACE(t.Empressa, CHAR(160), ' '),CHAR(9), ''),CHAR(13), ''),CHAR(10), '')))) AS NomeEmpresa,
            t.Porte,
            t.DataUltimoDiaCorrecao,
            LTRIM(RTRIM(UPPER(p.Nome))) AS Analista_dbm
        FROM
            TtbuladorOnboardingPJ_Manual t WITH(NOLOCK)
...
# ==============================
def executar_e_salvar_tabulacoes_OnboardingPJ_Manual():
    print("Iniciando processo de extracao Tabulacoes OnboardingPJ_Manual...")
    
    try:
        # Carrega os dados
        df = carregar_tabulacoes_OnboardingPJ_Manual()

        if df.empty:
            print("A consulta nao retornou dados para o periodo informando no arquivo de aux_data_sistema.py")
        else:
            # Caminhos com subpasta
            caminho_parquet = os.path.join(PASTA_DESTINO, "import_tabulacoes_OnboardingPJ_Manual.parquet")
            # caminho_excel = os.path.join(PASTA_DESTINO, "import_tabulacoes_OnboardingPJ_Manual.xlsx")
...
if __name__ == "__main__":
    executar_e_salvar_tabulacoes_OnboardingPJ_Manual()