...
def carregar_tabulacoes_n3_emissores():
    ...
    with engine.connect() as cnxn:
        ...
        FROM
            TbaseTabuladorEmissores b WITH(NOLOCK) 
            LEFT JOIN TtbuladorEmissor t WITH(NOLOCK) ON b.Id = t.FkTbaseTabuladorEmissores
            LEFT JOIN TemissoresDescricao d WITH(NOLOCK) ON b.FkTipoEmissor = d.Id
...
# ==============================
def executar_e_salvar_tabulacoes_n3_emissores():
    print("Iniciando processo de extracao e salvamento Tabulacoes N3 Emissores...")
    
    try:
        # 1. Obter dados novos do SQL
        df_novo = carregar_tabulacoes_n3_emissores()
        
        if df_novo.empty:
            print("Nenhum dado novo retornado para o período.")
            return

        caminho_parquet = os.path.join(PASTA_DESTINO, "import_tabulacoes_n3_emissores.parquet")
...
if __name__ == "__main__":
    executar_e_salvar_tabulacoes_n3_emissores()