...
# ==============================
def carregar_tabulacoes_n2_falcon():
    
    engine = login.connProDockTech
    with engine.connect() as cnxn:
        query_sql = f"""
        SELECT
            'N2: Falcon' AS Tabulador,
            t.Id,
            t.DtCadastro AS Tabulacao_DtCadastro,
            DATEADD(SECOND,DATEDIFF(SECOND, CAST('00:00:00' AS TIME), t.HoraDaEntrada), CAST(t.DataDaEntrada AS DATETIME2)) AS DataHoraEntrada,
            LTRIM(RTRIM(UPPER(t.Status))) AS Status,
            LTRIM(RTRIM(UPPER(t.MotivoDoStatus))) AS MotivoDoStatus,
            LTRIM(RTRIM(UPPER(p.Nome))) AS Nome
        FROM
            TtbuladorFalcon t WITH(NOLOCK)
...
def executar_e_salvar_tabulacoes_n2_falcon():
    print("Iniciando processo de extracao e salvamento Tabulacoes N2 Falcon...")
    
    try:
        # 1. Obter dados novos do SQL
        df_novo = carregar_tabulacoes_n2_falcon()
        
        if df_novo.empty:
            print("Nenhum dado novo retornado para o período.")
            return

        caminho_parquet = os.path.join(PASTA_DESTINO, "import_tabulacoes_n2_falcon.parquet")
...
if __name__ == "__main__":
    executar_e_salvar_tabulacoes_n2_falcon()
